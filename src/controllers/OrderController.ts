...
const createCheckoutSession = async (req:Request,res:Response) => {
    ...
    try {
        ...
        if (!session.url) {
            return res.status(500).json({message:"Fail Create Stripe Session"});
        }
        await newOrder.save();
        return res.json({url:session.url});
    } catch (error:any) {
        console.log(error);        
        return res.status(500).json({message:error.row.message})
    }
}
...
//         cancel_url:`${FRONTEND_URL}/detail/${restaurantId}?canselled=true`
